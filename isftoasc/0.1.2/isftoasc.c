...
*     Do not contact or ask Tektronix(c) about this program.         *
*                                                                    *
* DEC 04 2002  Ver. 0.1.2       Yohei ENDO <yoheie@ap3.mopera.ne.jp> *
*********************************************************************/
#include<stdio.h>
#include<string.h>
#include<stdlib.h>
...
int main(int argc, char *argv[]){
	...
	FILE *isffile;
	int i=0;
	char c, header_string[MAXHEADERSIZE+1],check_str[14];
	...
	if((isffile=fopen(argv[1],"r"))==NULL){
  ...
  /***** Check if the File is ISF format (6 lines) *****/
  ...
  /***** Read Header to header_string[] (10 lines) *****/
  ...
  /***** Init and Check Header (4 lines, initisfheader, checkisfheader) *****/
	if((initisfheader(header_string))!=0 || (checkisfheader())!=0){
		fclose(isffile);
		return 1;
	}

  /***** Read, Calc Data and Print (18 lines, read_curve_bin) *****/
  ...
  /***** Check if isffile is finished (5 lines) *****/
